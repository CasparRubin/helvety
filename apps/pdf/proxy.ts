...
function redirectRootToPdfBasePath(request: NextRequest): NextResponse | null {
  const incomingPathname = new URL(request.url).pathname;
  if (incomingPathname !== "/") {
    return null;
  }

  const basePath = request.nextUrl.basePath || "/pdf";
  return NextResponse.redirect(new URL(basePath, request.url));
}

/** Redirect direct-domain root requests before applying shared security middleware. */
export async function proxy(request: NextRequest) {
  const rootRedirect = redirectRootToPdfBasePath(request);
  if (rootRedirect) {
    return rootRedirect;
  }
