...
- Read paths use authenticated read model with rate limiting. List/detail prefetch GET routes use `@helvety/shared/encrypted-prefetch-api` (`RATE_LIMITS.PREFETCH`, `ENCRYPTED_PREFETCH_COLUMNS`). Cross-app link pickers (`getContacts`, `getTaskEntities`, `getLinkEntities`) cap at `ACTION_LIMITS.MAX_DASHBOARD_ROWS` (**2,000** rows per query). The contact link picker loads the full `ENCRYPTED_PREFETCH_COLUMNS.contacts` list (Tasks uses the slimmer `CONTACT_LINK_PICKER_COLUMNS` subset; see `encrypted-prefetch-api.ts`).
