...
describe("image upscaler SEO routes", () => {
  it("returns crawlable robots with expected disallow rules", () => {
    ...
    expect(disallowPaths).not.toContain("/image-upscaler");
    expect(disallowPaths).not.toContain("/image-upscaler/sitemap.xml");
    expect(robotsOutput.sitemap).toBe(
      `${urls.home}/image-upscaler/sitemap.xml`
    );
  });
  ...
  it("returns canonical sitemap entries", () => {
    ...
    expect(entries).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ url: urls.imageUpscaler }),
        expect.objectContaining({ url: `${urls.imageUpscaler}/llms.txt` }),
      ])
    );
  });
});
