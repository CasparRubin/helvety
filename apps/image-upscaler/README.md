...
- AI inference runs entirely in a Web Worker via `onnxruntime-web` (`webgpu` -> `wasm` execution providers); [`lib/upscale-worker-client.ts`](./lib/upscale-worker-client.ts) enforces a **180s** per-request timeout, terminates the worker on crash/timeout, and recreates it on the next call. The UI aborts in-flight batches on unmount or when starting a new run.
...
- The first AI run fetches the Real-ESRGAN weights from the public Supabase Storage bucket configured for the active environment, then caches them locally. When `sha256` is set in [`lib/models.ts`](./lib/models.ts), downloads are verified before use; if **Web Crypto** (`crypto.subtle`) is unavailable, verification **fails closed** and inference does not use unverified bytes (see [`lib/model-cache.ts`](./lib/model-cache.ts)).
