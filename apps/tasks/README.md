...
- New and edit use the same wide right detail sheet (`E2eeEntityDetailSheet`; flex scroll chain via `@helvety/ui/sheet-scroll-layout`, body scrolls in `CommandBarPageLayout`) with the full `ItemEditor` (Tiptap, metadata panel, contact/note/link cross-app panels). Start/End dates in the metadata panel (`item-action-panel`) use `@helvety/ui/form-field` + `@helvety/ui/date-time-picker` with stable `start-date` / `end-date` ids. **New Task** uses save-first create: `openCreate()` opens the sheet in `formMode="create"` with empty defaults from `@helvety/shared/e2ee-create-inputs` (no list row, no URL until save); first save calls `create()` (client UUID at encrypt time), then `openEntity(id)` for edit mode and `?item=`. Cross-link panels appear only in edit mode. Sheet entity resolution uses `useE2eeDashboardSelectedEntity` (list match or single-row fetch for deep links).
...
- `/tasks/robots.txt` disallows `/tasks` (zone mirror; canonical crawl policy is gateway `/robots.txt`).
- `/tasks/sitemap.xml` is not published (404). Private zones omit sitemap routes. Zone `llms.txt` remains published and is linked from the gateway guide, but gateway `robots.txt` disallows `/tasks` so compliant crawlers should not fetch it.
