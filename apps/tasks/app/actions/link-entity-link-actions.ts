 ...
 *
 */
export interface ItemLinkEntityLinkData {
  links: LinkedLinkRow[];
}
 ...
 *
 */
export async function getItemLinkEntityLinks(
  itemId: string
): Promise<ActionResponse<ItemLinkEntityLinkData>> {
  try {
    ...
    if (linksResult.error) {
      logger.logUnexpectedError(
        "Error getting bookmark links for task",
        linksResult.error
      );
    ...
    return unexpectedActionError(
      "Unexpected error in getItemLinkEntityLinks",
      error
    );
  }
}
...
export async function linkLinkEntity(
  ...
  try {
    ...
    const linkResult = await createCanonicalLink({
      ...
      duplicateMessage: "Link is already linked to this task",
    ...
    if (!linkResult.success) {
      if (linkResult.logError) {
        logger.logUnexpectedError(
          "Error linking bookmark to task",
          linkResult.logError
        );
      }
