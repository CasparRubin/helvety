 ...
 * open redirect attacks. Invalid URIs fall back to the tasks app URL.
 */
export function getLoginUrl(currentUrl?: string): string {
  ...
  if (currentUrl && isValidRedirectUri(currentUrl)) {
    ...
    redirectUri = isValidRedirectUri(windowUrl) ? windowUrl : urls.tasks;
  } else {
    redirectUri = urls.tasks;
  }
...
export function getLogoutUrl(redirectUri?: string): string {
  const redirect =
    redirectUri && isValidRedirectUri(redirectUri) ? redirectUri : urls.tasks;
