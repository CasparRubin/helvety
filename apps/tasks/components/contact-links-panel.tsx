...
function useContactLinksAdapter(
  entityId: string,
...
const CONTACT_LINK_LABELS = {
  ...
  unlinkDescription: (name: string) =>
    `Are you sure you want to unlink "${name}" from this task? The contact itself will not be deleted.`,
};

/** Panel for linking/unlinking contacts to a task. */
export function ContactLinksPanel({
  ...
  return (
    <EntityLinksPanel<Contact, LinkedContact>
      ...
      useLinks={useContactLinksAdapter}
    />
  );
}
