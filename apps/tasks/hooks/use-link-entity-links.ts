...
>({
  ...
  source: "tasks-use-link-entity-links",
  messages: {
  ...
  loadLinks: getItemLinkEntityLinks,
  ...
  link: (itemId, linkId, csrfToken) =>
    linkLinkEntity(linkId, itemId, csrfToken),
  unlink: unlinkLinkEntity,
});

/** Hook to fetch, decrypt, link, and unlink bookmarks for a task. */
export function useLinkEntityLinks(
  itemId: string,
  options?: { enabled?: boolean }
) {
  return useLinkEntityLinksHook(itemId, options);
}
