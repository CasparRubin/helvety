 ...
 * alpha canvas / bloom never shows page chrome before the reveal wrapper fades in; pairs
 * with {@link ../hero-hyperspeed-backdrop.tsx}. See {@link ./Hyperspeed.css} for `#lights`
 * background (`var(--background)`; `.dark` uses `#1c1816` during chunk load / dispose).
 */
import {
  BloomEffect,
  EffectComposer,
...
interface HyperspeedOptions {
  onSpeedUp?: (ev?: MouseEvent | TouchEvent | PointerEvent) => void;
  onSlowDown?: (ev?: MouseEvent | TouchEvent | PointerEvent) => void;
...
class App {
  ...
  /** @returns Whether postprocessing passes were registered (composer ready to render). */
  initPasses(): boolean {
  ...
  attachGlobalInteractRelease(): void {
    window.addEventListener("pointerup", this.onGlobalInteractRelease, true);
    window.addEventListener(
      "pointercancel",
      this.onGlobalInteractRelease,
      true
    );
    window.addEventListener("mouseup", this.onGlobalInteractRelease, true);
    window.addEventListener("touchend", this.onGlobalInteractRelease, true);
    window.addEventListener("touchcancel", this.onGlobalInteractRelease, true);
  }

  detachGlobalInteractRelease(): void {
    window.removeEventListener("pointerup", this.onGlobalInteractRelease, true);
    window.removeEventListener(
      "pointercancel",
      this.onGlobalInteractRelease,
      true
    );
    window.removeEventListener("mouseup", this.onGlobalInteractRelease, true);
    window.removeEventListener("touchend", this.onGlobalInteractRelease, true);
    window.removeEventListener(
      "touchcancel",
      this.onGlobalInteractRelease,
      true
  ...
  beginInteractBoost(ev?: MouseEvent | TouchEvent | PointerEvent): void {
    ...
    this.interactionBoostActive = true;
    this.attachGlobalInteractRelease();
    this.options.onSpeedUp?.(ev);
    this.fovTarget = this.options.fovSpeedUp;
    this.speedUpTarget = this.options.speedUp;
  }

  endInteractBoost(ev?: MouseEvent | TouchEvent | PointerEvent): void {
    if (!this.interactionBoostActive) return;
