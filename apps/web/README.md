...
- **Home (`/`):** [`HeroMarketingShell`](components/hero-marketing-shell.tsx) from [`app/page.tsx`](app/page.tsx) server-renders static eyebrow (`text-base`), headline (Swiss origin accent), a simple company-values tagline (`private · simple · clean` from `HELVETY_COMPANY_VALUES_TAGLINE`) via [`HeroCompanyValuesTagline`](components/hero-company-values-tagline.tsx), and a responsive two-CTA row: primary **Helvety Cloud** (`urls.cloud` → `https://helvety.cloud`) plus outline **Browse products** deep-linked to `urls.storeProducts` (`/store/products`) on a plain `bg-background`. The page also mounts [`StoreProductsSpeculation`](components/store-products-speculation.tsx). The hero background is just black or white, depending on the resolved theme. React Bits **Shuffle / ShinyText** presets live in [`hero-text.tsx`](components/hero-text.tsx) for vendor refresh and tests; production `/` does **not** mount them.
