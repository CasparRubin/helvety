...
- Marketing home page (`/`): [`components/hero-section.tsx`](components/hero-section.tsx) centers copy and CTA in the first main viewport band; [`app/globals.css`](app/globals.css) (`.hero-bg-pattern`) applies a viewport-centered radial mask over an animated dot grid; from `md` up a second column shows the floating Helvety identifier (hidden on smaller breakpoints)
