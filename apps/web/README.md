...
- Marketing home page (`/`): [`components/hero-section.tsx`](components/hero-section.tsx) centers copy and CTA in the first main viewport band. [`app/globals.css`](app/globals.css) styles the dot grid (`.hero-bg-pattern`), a rotating conic “shine” ring on `.hero-identifier-float::before`, and the floating identifier tile (orbs, breathe on `.helvety-identifier-mark`, dark-mode tweak for `.helvety-identifier-edge-shine`). [`packages/brand/src/identifier.tsx`](../../packages/brand/src/identifier.tsx) provides `HelvetyIdentifier` with optional `edgeHighlight` (gradient stroke under the fill, SVG SMIL when enabled). The hero passes `edgeHighlight` only when `useReducedMotion()` is strictly `false`, so both the ring animation and edge stroke stay off under `prefers-reduced-motion: reduce`. From `md` up a second column shows the identifier; it stays hidden on smaller breakpoints.
