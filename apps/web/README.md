...
React Bits motion sources live under [`components/vendor/`](components/vendor/) (`Hyperspeed.tsx`, `ElectricBorder.tsx`, `Shuffle.tsx`, `ShinyText.tsx`). Production `/` uses **server-rendered** copy in [`hero-marketing-shell.tsx`](components/hero-marketing-shell.tsx) (including middle-dot company-values tagline in [`HeroCompanyValuesTagline`](components/hero-company-values-tagline.tsx) / `ElectricBorder` from `HELVETY_COMPANY_VALUES_TAGLINE`) plus client WebGL in [`hero-hyperspeed-layer.tsx`](components/hero-hyperspeed-layer.tsx) / [`hero-hyperspeed-backdrop.tsx`](components/hero-hyperspeed-backdrop.tsx). [`hero-text.tsx`](components/hero-text.tsx) holds **Shuffle / ShinyText presets** for vendor refresh and unit tests ([`hero-text.test.tsx`](components/hero-text.test.tsx)); it is **not** imported by the production shell. Shell and WebGL tests: [`hero-marketing-shell.test.tsx`](components/hero-marketing-shell.test.tsx), [`hero-hyperspeed-layer.test.tsx`](components/hero-hyperspeed-layer.test.tsx), [`hero-hyperspeed-backdrop.test.tsx`](components/hero-hyperspeed-backdrop.test.tsx), [`hero-legacy-guard.test.ts`](components/hero-legacy-guard.test.ts), [`Hyperspeed.init-error.test.tsx`](components/vendor/Hyperspeed.init-error.test.tsx), [`Hyperspeed.visibility.test.tsx`](components/vendor/Hyperspeed.visibility.test.tsx), [`ElectricBorder.test.tsx`](components/vendor/ElectricBorder.test.tsx). **`Hyperspeed.tsx`** uses Three.js + `postprocessing` (SMAA optional fallback); **`ElectricBorder.tsx`** uses canvas 2D; **`Shuffle.tsx`** uses GSAP; **`ShinyText.tsx`** imports **`framer-motion`** (not `motion/react`). Vitest mocks `framer-motion` in hero tests and stubs or tests Hyperspeed/Shuffle/ElectricBorder separately. ESLint relaxes style rules for `components/vendor/**` in `@helvety/config` (no per-file `eslint-disable`).
...
```bash
bunx shadcn add @react-bits/ShinyText-TS-TW @react-bits/Shuffle-TS-TW @react-bits/ElectricBorder-JS-CSS
```

Reconcile Helvety tweaks in [`hero-text.tsx`](components/hero-text.tsx) and [`hero-company-values-tagline.tsx`](components/hero-company-values-tagline.tsx) after refresh. [**Hyperspeed**](components/vendor/Hyperspeed.tsx) is maintained manually in `components/vendor/` (not the shadcn text registry).
