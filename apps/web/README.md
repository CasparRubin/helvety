...
- Multi-zone gateway rewrites for `/auth`, `/store`, `/pdf`, `/docs`, `/image-upscaler`, `/tasks`, `/contacts`, `/notes`, `/links`
...
- Sensitive auth/data enforcement remains in app-specific zones (`auth`, `store`, `docs`, `tasks`, `contacts`, `notes`, `links`).
...
| `DOCS_URL`                             | Vercel production (`VERCEL=1`) | Yes         | Internal Vercel URL for docs zone rewrites      |
...
React Bits motion sources live under [`components/vendor/`](components/vendor/) (`Hyperspeed.tsx`, `Shuffle.tsx`, `ShinyText.tsx`). Production `/` composes hero copy in [`hero-marketing-shell.tsx`](components/hero-marketing-shell.tsx) with presets from [`hero-text.tsx`](components/hero-text.tsx) (tests in [`hero-text.test.tsx`](components/hero-text.test.tsx)). [`hero-section.tsx`](components/hero-section.tsx) is a legacy all-client harness for Vitest and WebGL tuning only (not used on `/`). **`Hyperspeed.tsx`** uses Three.js + `postprocessing`; **`Shuffle.tsx`** uses GSAP; only **`ShinyText.tsx`** imports **`framer-motion`** (not `motion/react`). `hero-text.tsx` and the legacy `hero-section` harness use `framer-motion` for `MotionConfig` / `useReducedMotion`; Vitest mocks `framer-motion` in those hero tests and stubs or tests Hyperspeed/Shuffle separately. ESLint relaxes style rules for `components/vendor/**` in `@helvety/config` (no per-file `eslint-disable`).
