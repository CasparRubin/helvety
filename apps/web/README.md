...
- **Home (`/`):** [`HeroSection`](components/hero-section.tsx) from [`app/page.tsx`](app/page.tsx) — Store CTA, WebGL [**Hyperspeed**](components/Hyperspeed.tsx) via `next/dynamic` when **reduced motion is off**, solid background otherwise. Backdrop uses **`cursor-grab` / `active:cursor-grabbing`** (press or hold to boost speed — no zoom-lens cursor). Tagline (`private · simple · clean`) uses a **text-only shimmer** (`hero-tagline-glow`) with reduced-motion fallback. CTA keeps **intrinsic width** on all breakpoints (no mobile `w-full`). Tunables: [`hyperspeed-default-preset.ts`](components/hyperspeed-default-preset.ts), [`hero-hyperspeed-options.ts`](components/hero-hyperspeed-options.ts), lateral mask [`hero-hyperspeed-bleed.css`](components/hero-hyperspeed-bleed.css).
- **Public shell:** [`app/layout.tsx`](app/layout.tsx) uses `HelvetyPublicShellRootLayout` (`mainVariant: "scroll-area"`, Speed Insights). Optional `shellColumnClassName`, `scrollAreaRootClassName`, `scrollAreaViewportClassName`, and `bodyClassName` let the hero canvas span the viewport horizontally without Radix clipping; documented on [`packages/ui/README.md`](../../packages/ui/README.md).
