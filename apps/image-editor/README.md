...
- Border, highlight, and blur regions support straight or rounded corners (`cornerRadius`); rounded highlights use a composite cutout overlay, straight highlights use dim strip geometry ([`lib/spotlight-rects.ts`](lib/spotlight-rects.ts) / [`lib/spotlight-cutout.ts`](lib/spotlight-cutout.ts))
