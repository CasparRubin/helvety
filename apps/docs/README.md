...
- **Vault bookmarks (`?doc=`):** `https://helvety.com/docs?doc=<uuid>` can appear in shared links and auth return URLs. The editor **always starts with a blank page** on load (including when `?doc=` is present); open saved documents from **My documents** in the title bar sheet after sign-in and vault unlock. The URL updates to `?doc=<uuid>` when you open or save a vault document from the app (not on initial landing).
...
- **Single toolbar:** document actions (New, Open, Download, My documents, Save to vault) live in Eigenpal’s title bar right slot via `renderTitleBarRight`; vendor **File → Open/Save** are hidden (File still has **Print** and **Page setup**) so open stays parent-controlled (`documentBuffer` + vault state) and export uses validated `onSave` (Cmd+S and the Download button), not Eigenpal’s menu save path.
- **Public route:** [`app/page.tsx`](./app/page.tsx) stays public; vault is a sheet opened from the title bar when signed in. Auth does not gate the blank editor.
...
- **Eigenpal editor chrome** (title bar with **File**, **Format**, and **Insert** menus plus Helvety actions in `renderTitleBarRight`, formatting toolbar, dialogs, workspace gutter) is themed in [`styles/docx-editor-helvety-bridge.css`](./styles/docx-editor-helvety-bridge.css), imported from [`app/globals.css`](./app/globals.css) after vendor styles. The bridge sets HSL channel variables on `.ep-root` aligned with [`packages/ui/globals.css`](../../packages/ui/globals.css); title-bar and formatting toolbar rows use `--surface-toolbar` and matching borders (title bar is the top chrome row; no separate Helvety command bar). Maintainer token reference (tests): [`lib/docx-editor-theme-tokens.ts`](./lib/docx-editor-theme-tokens.ts). The bridge hides Eigenpal’s default title-bar **doc icon**, **Help** menu, and internal docx file input; vault/My documents/New/Open/Download live in the title bar right slot ([`hooks/use-hide-vendor-file-menu-items.ts`](./hooks/use-hide-vendor-file-menu-items.ts) hides vendor File → Open/Save). **Comment** UI is suppressed. Dropdowns and tooltips use the same semantic popover/accent tokens in light and dark (Layer 8).
...
7. Eigenpal title bar: **File**, **Format**, and **Insert** visible; no default doc icon column, no **Help** menu, no **comment** UI; Helvety **New/Open/Download/My documents/Save to vault** in the title bar right slot; vendor File → Open/Save hidden
8. Single toolbar stack: no Helvety command row above the editor; title bar is top chrome (top border + side borders); formatting bar flush below (no color band or gap)
