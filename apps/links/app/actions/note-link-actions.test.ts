...
/** Builds a minimal Supabase mock for bookmark-note link actions. */
function createSupabaseMock() {
  const noteInReturns = vi.fn().mockResolvedValue({
    data: [{ id: "note-1", encrypted_title: "enc-title" }],
    error: null,
  });
  const noteInEqUser = vi.fn(() => ({ overrideTypes: noteInReturns }));
  const noteIn = vi.fn(() => ({ eq: noteInEqUser }));

  const from = vi.fn((table: string) => {
    if (table === "notes") {
      return { select: () => ({ in: noteIn }) };
    }
    if (table === "links") {
      return {
        select: () => ({
...
describe("links note-link-actions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("validates bookmark id before auth for getLinkNoteLinks", async () => {
    const result = await getLinkNoteLinks("invalid-id");
  ...
  it("returns linked notes and supports link/unlink operations", async () => {
    ...
    const links = await getLinkNoteLinks(
