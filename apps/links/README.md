...
- New and edit use the same wide right detail sheet (`E2eeEntityDetailSheet`; flex scroll chain via `@helvety/ui/sheet-scroll-layout`, body scrolls in `CommandBarPageLayout`) with `LinkEditor` / `FolderEditor` (`dynamic(..., { ssr: false })`) and `LinksEditorCommandBar` (wraps shared `EditorCommandBar`: save, refresh, unsaved-change detection, delete in overflow). **New link** / **New folder** use the same open-first pattern as tasks/notes/contacts (`seedLinkDraft` / `seedFolderDraft` → open sheet → first save via `updateLink` / `updateFolder` → `createLinkWithId` / `createFolderWithId` with a client-generated id); closing an unchanged draft removes the optimistic row (`isPendingLinkDraft` / `isPendingFolderDraft` → `removeLinkDraft` / `removeFolderDraft`, no server row). The link editor includes cross-app panels to tasks, contacts, and notes (`EntityLinksPanel` + `createE2eeEntityLinksHook`; catalog/link fetches are deferred until a panel expands or the Add picker opens).
