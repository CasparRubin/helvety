...
- New and edit use the same wide right detail sheet (`E2eeEntityDetailSheet`; flex scroll chain via `@helvety/ui/sheet-scroll-layout`, body scrolls in `CommandBarPageLayout`) with `LinkEditor` / `FolderEditor` (`dynamic(..., { ssr: false })`) and `LinksEditorCommandBar` (wraps shared `EditorCommandBar`: save, refresh, unsaved-change detection, delete in overflow). Link and folder forms use `@helvety/ui/form-field` (`link-form-fields`, `folder-editor` with stable `folder-name` / `folder-parent` ids). **New link** / **New folder** use save-first create (`formMode="create"` with `emptyLinkInput` / `emptyLinkFolderInput` from `@helvety/shared/e2ee-create-inputs`; panel `{ mode: "create" }` without URL; first save calls `library.createLink` / `library.createFolder`, which generate a client UUID at save time). After create, the sheet switches to edit mode and syncs `?link=` / `?folder=`. The link editor includes cross-app panels to tasks, contacts, and notes in edit mode only (`EntityLinksPanel` + `createE2eeEntityLinksHook`; catalog/link fetches are deferred until a panel expands or the Add picker opens).
...
- Detail sheet saves: dashboard passes `library.updateLink` / `library.updateFolder` and `library.refresh` into `LinkEditor` / `FolderEditor` (Links pattern: single library state; list patches after network success, unlike sortable-items optimistic `update()`).
