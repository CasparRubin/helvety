...
**Login client bootstrap** (`hooks/use-login-flow.ts`): on mount, probes the Supabase session once (Strict Mode–safe one-shot) via `getUserSingleflight` with a short timeout. After OTP success, `invalidateAuthUserProbeCache()` runs before CSRF sync so the probe sees the new session. Stale bootstrap errors are suppressed after OTP (`shouldSurfaceLoginError` in `lib/login-flow-errors.ts`); auto-passkey `NotAllowedError` / `AbortError` on mobile are silent so dismissals do not toast false failures.
