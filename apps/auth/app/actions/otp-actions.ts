...
import { AUTH_ACTIONS, logAuthEvent } from "@helvety/shared/auth-logger";
...
> {
  ...
  const preCheck = await runOtpVerifyRateLimits(
    normalizedEmail,
    clientIP,
    AUTH_ACTIONS.verifyEmailCode
  );
  if (!preCheck.success) {
    return preCheck;
  }
