...
/** Builds a minimal Supabase mock for contact-bookmark link actions. */
...
describe("contacts link-entity-link-actions", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("validates contact id before auth for getContactLinkEntityLinks", async () => {
    const result = await getContactLinkEntityLinks("invalid-id");
  ...
  it("returns linked bookmarks and supports link/unlink operations", async () => {
    ...
    const links = await getContactLinkEntityLinks(
