 ...
 *
 */
export interface ContactLinkEntityLinkData {
  links: LinkedLinkRow[];
}
 ...
 *
 */
export async function getContactLinkEntityLinks(
  contactId: string
): Promise<ActionResponse<ContactLinkEntityLinkData>> {
  try {
    ...
    if (linksResult.error) {
      logger.logUnexpectedError(
        "Error getting bookmark links for contact",
        linksResult.error
      );
    ...
    return unexpectedActionError(
      "Unexpected error in getContactLinkEntityLinks",
      error
    );
  }
}
...
export async function linkLinkEntity(
  ...
  try {
    ...
    const linkResult = await createCanonicalLink({
      ...
      duplicateMessage: "Link is already linked to this contact",
    ...
    if (!linkResult.success) {
      if (linkResult.logError) {
        logger.logUnexpectedError(
          "Error linking bookmark to contact",
          linkResult.logError
        );
      }
