...
>({
  ...
  link: (contactId, noteId, csrfToken) =>
    linkNoteEntity(noteId, contactId, csrfToken),
  unlink: unlinkNoteEntity,
});

/** Hook to fetch, decrypt, link, and unlink notes for a contact. */
export function useNoteLinks(
  contactId: string,
  options?: { enabled?: boolean }
) {
  return useNoteLinksHook(contactId, options);
}
