...
- New and edit use the same wide right detail sheet (`E2eeEntityDetailSheet`; flex scroll chain via `@helvety/ui/sheet-scroll-layout`, body scrolls in `CommandBarPageLayout`) with `ContactEditor` on `@helvety/ui/e2ee-item-editor-shell` (structured fields via `@helvety/ui/form-field`, Tiptap notes, task/note/link cross-app panels). **New Contact** uses save-first create: `openCreate()` opens the sheet in `formMode="create"` with empty defaults from `@helvety/shared/e2ee-create-inputs` (no list row, no URL until save); first save validates via `@helvety/shared/validate-e2ee-draft` (required first name), then calls `create()` (client UUID at encrypt time), then `openEntity(id)` for edit mode and `?contact=`. Cross-link panels appear only in edit mode. Sheet entity resolution uses `useE2eeDashboardSelectedEntity`.
...
- `/contacts/robots.txt` disallows `/contacts` (zone mirror; canonical crawl policy is gateway `/robots.txt`).
- `/contacts/sitemap.xml` is not published (404). Private zones omit sitemap routes. Zone `llms.txt` remains published and is linked from the gateway guide, but gateway `robots.txt` disallows `/contacts` so compliant crawlers should not fetch it.
