...
- Third-party React Bits text sources (**RotatingText**, **Shuffle**, **ShinyText**) live in `apps/web/components/vendor/`, not the shared `packages/ui` package.
- Gateway hero: production `/` uses `hero-marketing-shell.tsx` with a **RotatingText** client island (`HeroSwitzerlandHeadline`: Engineered / Designed / Made + static Switzerland accent; reduced-motion static fallback) and company-values tagline as simple middle-dot text `private · simple · clean` from `HELVETY_COMPANY_VALUES_TAGLINE` in `@helvety/shared/licensing` via `hero-company-values-copy.ts`, Store CTA deep-linked to `urls.storeProducts`, soft radial atmosphere over `bg-background`. `hero-text.tsx` holds Shuffle/ShinyText presets for vendor refresh and tests only; it is not wired into the production shell. Public shells inject blocking theme init in `<head>` (not after `SkipToContent`), so the gateway hero background resolves with the active theme.
...
- After `shadcn add @react-bits/...` from `apps/web`, reconcile Helvety tweaks (prefer `framer-motion` over `motion/react`) and keep motion files under `components/vendor/` with imports updated in `hero-switzerland-headline.tsx` / `hero-text.tsx`.
...
iOS Safari zooms the page when a focused text field is below **16px**. Helvety uses **one shared system** in `@helvety/ui`. Do not duplicate font-size classes in apps.
