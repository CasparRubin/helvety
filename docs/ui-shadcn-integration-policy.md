...
- Gateway hero: production `/` uses server-rendered copy in `hero-marketing-shell.tsx` (eyebrow at `text-base`, headline, company-values tagline as three shadcn **`Badge`** pills from `@helvety/ui/badge` with Lucide icons, Store CTA) plus client WebGL in `hero-hyperspeed-layer.tsx` / `hero-hyperspeed-backdrop.tsx`. Tagline labels are derived from `HELVETY_COMPANY_VALUES_TAGLINE` in `@helvety/shared/licensing` (one pill per value; Lucide icons OK; no emoji). `hero-text.tsx` holds Shuffle/ShinyText presets for vendor refresh and tests only; not wired into the production shell. `HeroHyperspeedLayer` skips the bleed host when `prefers-reduced-motion: reduce` **or** `canUseWebGL()` from `@helvety/light-pillar` is false. The backdrop keeps Hyperspeed at `opacity-0` until the first composited WebGL frame with a matching `html.dark`, then fades in over **2000ms**; it hides before cross-zone links and `pagehide`, re-reveals when the tab becomes visible again, and remounts WebGL after bfcache restore. Do not reintroduce a lifting “veil” over a visible canvas. Public shells inject blocking theme init in `<head>` (not after `SkipToContent`). Gateway overflow escape props (`!overflow-visible` on the public shell scroll column) apply **on `/` only** via `getGatewayShellLayoutProps` and the proxy’s `x-helvety-pathname` header; legal and other subpages keep default scroll clipping.
