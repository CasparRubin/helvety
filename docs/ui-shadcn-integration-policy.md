...
- Pin command bars **outside** scroll: E2EE list/editor pages wrap toolbar + body in `CommandBarPageLayout` (scrolls via `@helvety/ui/scroll-area`); Store uses `scrollAreaMainPrefix` on `HelvetyPublicShellRootLayout`; PDF/image-upscaler pin `CommandBar` as a flex sibling above an `overflow-hidden` workspace. Do not rely on CSS `sticky` on `CommandBar` for page-level pinning.
