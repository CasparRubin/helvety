...
| Open-first create     | Client UUID at click → `seedDraft` / `seedLinkDraft` → open sheet → background `createWithId` with that id (E2EE v2 field-bound AAD). Cleanup unchanged drafts with `removeDraft` while `isOpeningDraft`, else server `remove`. Links uses local panel state + `useLinksPanelUrlSync` instead of `useE2eeEntityPanelWithUrl`.                                                                            | `tasks`, `notes`, `contacts`, `links`                              |
