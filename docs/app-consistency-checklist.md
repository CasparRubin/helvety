...
| Open-first create     | Client UUID at click → `seedDraft` / `seedLinkDraft` / `seedFolderDraft` → open sheet (no background insert). First save calls `createWithId` / `createLinkWithId` / `createFolderWithId` with merged user input (E2EE v2 field-bound AAD). Cleanup unchanged or pre-save drafts with `removeDraft` / `removeLinkDraft` / `removeFolderDraft` while `isPendingDraft` / `isPendingLinkDraft` / `isPendingFolderDraft`, else server `remove`. Owned updates use `ownedUpdateMissingRow` after `.select("id").maybeSingle()`. Links uses local panel state + `useLinksPanelUrlSync` instead of `useE2eeEntityPanelWithUrl`. | `tasks`, `notes`, `contacts`, `links`                              |
