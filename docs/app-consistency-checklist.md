...
- Store catalog SSOT: `@helvety/shared/helvety-ecosystem-sections` (categories + app-switcher grouping) → `@helvety/shared/store-catalog` (card fields; `category` derived) → `apps/store/lib/data/products.ts` (full rows). Wiring: `helvety-ecosystem-sections.test.ts`, `store-catalog.test.ts`, `packages/ui/src/app-switcher-sections.test.ts`, `apps/store/components/products/product-ui-wiring.test.ts`.
