...
| Vault session TTL     | `auth-session-policy.ts` + `vault-session.ts` (24h sliding idle, 7d max); enforced in `encryption-context`, Chromium extension action popup, and IndexedDB                                                                                                                       | All zones using `EncryptionProvider` + Chromium extension          |
