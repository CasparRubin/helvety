...
| Playwright gateway smoke                                    | manual `test:e2e` + dev server   | **`bun run ci:check:e2e`** — installs Chromium, starts all zones via `scripts/run-e2e-smoke.mjs` when `HELVETY_SMOKE_BASE_URL` is unset, and uses non-production placeholder credentials when local service env is absent (optional; not in default `ci:check`) |
