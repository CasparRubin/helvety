...
| ---------------------------- | --------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gateway**                  | `web` (`helvety-com`)                   | `NEXT_PUBLIC_SUPABASE_*` (`sb_publishable_*` publishable key), nine zone rewrite URLs (`AUTH_URL`, `STORE_URL`, `PDF_URL`, `IMAGE_UPSCALER_URL`, `IMAGE_EDITOR_URL`, `TASKS_URL`, `CONTACTS_URL`, `NOTES_URL`, `LINKS_URL`) when `VERCEL=1`; no Upstash or cookie-signing secrets |
...
| **Public tool + rate limit** | `pdf`, `image-upscaler`, `image-editor` | `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (`sb_publishable_*`) + `UPSTASH_REDIS_*`, `HELVETY_COOKIE_SIGNING_SECRET` (auth callbacks use strict rate limiting)                                                                                                                        |
