...
| ---------------------------- | --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Gateway**                  | `web` (`helvety-com`)                         | `NEXT_PUBLIC_SUPABASE_*` (`sb_publishable_*` publishable key), nine zone rewrite URLs (`AUTH_URL`, `STORE_URL`, `PDF_URL`, `DOCS_URL`, `IMAGE_UPSCALER_URL`, `TASKS_URL`, `CONTACTS_URL`, `NOTES_URL`, `LINKS_URL`) when `VERCEL=1`; no Upstash or cookie-signing secrets |
...
| **User-scoped + rate limit** | `tasks`, `contacts`, `notes`, `links`, `docs` | `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (`sb_publishable_*`) + `UPSTASH_REDIS_*`, `HELVETY_COOKIE_SIGNING_SECRET`, **`DEVICE_TRUST_COOKIE_SECRET`** (same value as `auth`; weekly device-trust gate; no `SUPABASE_SECRET_KEY`; vault CRUD uses user client + RLS)          |
| **Public tool + rate limit** | `pdf`, `image-upscaler`                       | `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (`sb_publishable_*`) + `UPSTASH_REDIS_*`, `HELVETY_COOKIE_SIGNING_SECRET` (auth callbacks use strict rate limiting)                                                                                                                |
