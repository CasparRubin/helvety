...
| **User-scoped + rate limit** | `tasks`, `contacts`, `notes`, `links`, `docs` | `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (`sb_publishable_*`) + `UPSTASH_REDIS_*`, `HELVETY_COOKIE_SIGNING_SECRET` (no `SUPABASE_SECRET_KEY`; vault CRUD uses user client + RLS)                                                                                            |
