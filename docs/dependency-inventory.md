...
| Supabase JS client                   | `^2.110.2` (drift; security floor `2.110.2`; root override `2.110.2`)                                                                                                                                | supabase-js                    | https://github.com/supabase/supabase-js/releases                 | Bump in drift script + apps; `deps:security:floors`                              | Auth/API                    |
...
| `@base-ui/react`                     | `^1.6.0` (drift; `@helvety/ui` only)                                                                                                                                                                 | MUI Base UI                    | https://github.com/mui/base-ui/releases                          | Bump in drift script + `packages/ui`; regen shadcn primitives if API changes     | All UI surfaces             |
...
| `shadcn` (CSS + CLI)                 | `^4.13.0` — production dep on `@helvety/ui` (`globals.css` imports `shadcn/tailwind.css`); CLI also in `@helvety/dev-deps`                                                                           | shadcn/ui                      | Same as CLI row                                                  | Bump in drift script + `packages/ui`; extension Vite alias in `vite.config.ts`   | CSS build / extension popup |
| Root security overrides              | `hono@4.12.29`, `vite@8.1.4`, `rollup@4.62.2`, `protobufjs@7.6.4`, `dompurify@3.4.11`, `js-yaml@4.2.0`, `undici@7.28.0`, `@babel/core@8.0.1`, `postcss@8.5.16`, `qs@6.15.3`, `brace-expansion@5.0.7` | npm                            | `bun audit`                                                      | Bump root `overrides`; `bun install`                                             | Transitive CVEs             |
...
| PDF.js worker (vendored)       | `apps/pdf/public/pdf.worker.min.mjs`                              | `react-pdf>pdfjs-dist/build/` | Same as pdf.js                                  | `apps/pdf/scripts/sync-pdf-worker.mjs` on dev/build; writes `pdf.worker.meta.json`                                                                       | Stale worker vs lib |
