...
- **`lib/product-copy.ts`** (where used): **`pdf`** and **`image-upscaler`** re-export **`PDF_*`** / **`IMAGE_UPSCALER_*`** constants from `@helvety/shared/app-product-descriptions` (canonical strings live in shared; local file is for tests and manifest wiring).
...
| Encryption (non-E2EE)   | State **browser-local** / **not sent to Helvety** for local-only flows; never imply full-app E2EE for PDF, Image Upscaler, Store, or gateway                                                                                                                                      |
