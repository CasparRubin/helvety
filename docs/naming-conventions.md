...
- **`lib/product-copy.ts`** (where used): **`pdf`**, **`image-upscaler`**, **`image-editor`**, and **`ocr`** re-export **`PDF_*`** / **`IMAGE_UPSCALER_*`** / **`IMAGE_EDITOR_*`** / **`OCR_*`** constants from `@helvety/shared/app-product-descriptions` (canonical strings live in shared; local file is for tests and manifest wiring).
- **`public/manifest.json` `description`** (PWA install prompt): often matches the metadata string; when it must be shorter, use an explicit **`AUTH_PWA_MANIFEST_DESCRIPTION`** in `app/layout.tsx`, or **`PDF_PWA_MANIFEST_DESCRIPTION`** / **`IMAGE_UPSCALER_PWA_MANIFEST_DESCRIPTION`** / **`IMAGE_EDITOR_PWA_MANIFEST_DESCRIPTION`** in `lib/product-copy.ts`, and keep the JSON identical. **`bun run consistency:install-manifest-metadata`** ([`scripts/check-install-manifest-metadata.mjs`](../scripts/check-install-manifest-metadata.mjs)) fails if manifests drift from those constants (or from exported layout strings for apps without a separate PWA field).
...
- **`public/llms.txt`**: the `>` tagline is a hand-maintained crawler summary (company/product positioning only). Put license terms under **`## Licensing`** using `HELVETY_LLMS_LICENSING_NOTE` from `@helvety/shared/licensing`. Include **`## Crawling And Indexing`** (indexable vs noindex/auth-gated intent for agents; private zones should also note gateway `robots.txt` path disallows). Gateway `apps/web/public/llms.txt` links per-zone guides under **`## Agent And Crawler Guides`** (mark private-zone links as robots-disallowed). Vitest enforces em-dashes, a license-free tagline, crawling section, and licensing (`store-copy-guardrails.test.ts`). Public `llms.txt` is discoverable via robots allow rules and gateway links; **not** listed in Google sitemaps (sitemaps include indexable page URLs only). Private-zone `llms.txt` files remain published but those path prefixes are disallowed in gateway robots.
- **Robots and AI crawlers**: Compliant crawlers (RFC 9309 / Google) only honor gateway `https://helvety.com/robots.txt` from `apps/web` via `createOpenRobots("/sitemap-index.xml", GATEWAY_DISALLOWED_PATHS)`. That file allows public crawl for `*` and `AI_DISCOVERY_USER_AGENTS` (current OpenAI / Anthropic / Google / Perplexity / Apple tokens; training and search/user agents both allowed on public content) and disallows private zones plus non-indexable account/API paths. Path-zone `app/robots.ts` files are **mirrors** (`createAppRobots` prefixes zone-relative disallows to host-absolute paths; `createPrivateAppRobots("/auth")` etc. disallow the zone prefix). Private non-indexable zones (`auth` and E2EE vault apps) also use `indexing: "none"` and omit `app/sitemap.ts`. Public-zone `llms.txt` aids agentic discovery via robots allow rules and gateway links; private-zone `llms.txt` is published and gateway-linked but those prefixes are robots-disallowed. Metadata still uses `createHelvetyProductMetadata` (`indexing: "all" | "none"`, `googleBot` large previews). Public sitemaps use `createAppSitemap` or zone-specific `app/sitemap.ts` with indexable URLs only (`url` + `lastmod`; no `llms.txt`, `changefreq`, or `priority`).
...
| ---------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Ecosystem category** | `packages/shared/src/helvety-ecosystem-sections.ts` → section `title` + item `storeProductSlug`                                                                                                            | Encryption Apps, File Tools, Browser Extensions, SharePoint Apps, Desktop Apps. Drives app switcher grouping, store filter pills, and category badges (derived onto cards in `store-catalog.ts`). |
...
| Encryption (non-E2EE)   | State **browser-local** / **not sent to Helvety** for local-only flows; never imply full-app E2EE for PDF, Image Upscaler, Image Editor, OCR, Store, or gateway                                                                                                                                                                                                                                |
...
| Navbar About            | `@helvety/shared/app-navbar-about` (`*_NAVBAR_ABOUT`, `E2EE_NAVBAR_ENCRYPTION_TOOLTIP`) owns app-specific introduction copy only; the global About dialog in `HelvetyShellNavbar` renders a separate **Helvety** section for developer attribution, canonical Swiss origin, and generated version information (no license paragraph)                                                           |
