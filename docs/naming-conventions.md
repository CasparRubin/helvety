...
- **`lib/product-copy.ts`** (where used): **`pdf`**, **`image-upscaler`**, and **`image-editor`** re-export **`PDF_*`** / **`IMAGE_UPSCALER_*`** / **`IMAGE_EDITOR_*`** constants from `@helvety/shared/app-product-descriptions` (canonical strings live in shared; local file is for tests and manifest wiring).
