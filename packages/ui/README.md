...
- `@helvety/ui/loading-spinner` -> `LoadingSpinner`: Spinner only; root `app/loading.tsx` on **pdf** and **image-upscaler** re-export this directly. Nested routes (for example store product pages) may use `LoadingSpinner` as well.
