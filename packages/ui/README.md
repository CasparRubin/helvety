...
- `@helvety/ui/e2ee-item-editor-shell` -> `E2eeRichTextItemEditorShell`, `useE2eeRichTextItemEditorSave`: Shared rich-text editor shell (title, Tiptap body, unsaved-changes dialog). Tasks, notes, and contacts editors compose this shell with app-specific metadata and link panels. Pasted HTML is sanitized before Tiptap ingests it (`tiptap-paste-sanitize.ts`, DOMPurify + safe link schemes); link marks also block unsafe URL schemes in the editor.
