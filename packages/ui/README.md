...
- `@helvety/ui/e2ee-item-editor-shell` -> `E2eeRichTextItemEditorShell`, `useE2eeRichTextItemEditorSave`: Shared rich-text editor shell (title, Tiptap body, unsaved-changes dialog). Tasks, notes, and contacts editors compose this shell with app-specific metadata and link panels. Rich text is sanitized on paste and when loading stored JSON (`tiptap-paste-sanitize.ts`: DOMPurify for HTML, `sanitizeRichTextJson` strips unsafe link `href`s); the toolbar and `Link.validate` also block unsafe URL schemes.
