...
- `@helvety/ui/loading-spinner` -> `LoadingSpinner`: Spinner only; root `app/loading.tsx` on **E2EE apps**, **docs**, **pdf**, and **image-upscaler** re-export this directly (full-viewport `HelvetyShellRouteLoading` is reserved for gateway, auth, and store).
