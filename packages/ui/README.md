...
- `@helvety/ui/use-e2ee-entity-panel` -> `useE2eeEntityPanel`: Sheet open/close state and **open-first** **`openNewDraft`** (client UUID → `seedOptimistic` / dashboard `seedDraft` → open sheet; optional background `persist` — dashboards omit it and create on first save). Panel state only; no URL sync. `openEntity` / `closePanel` no-op when state is unchanged.
- `@helvety/ui/use-e2ee-entity-panel-with-url` -> `useE2eeEntityPanelWithUrl`, `useE2eeEntityUrlSync`: Tasks (`?item=`), Notes (`?note=`), and Contacts (`?contact=`). Writes the active entity id on open/close; wraps `openNewDraft` with URL + intent guards (dashboards omit `persist` — first save via list hook).
