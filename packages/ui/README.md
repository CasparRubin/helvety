...
- `@helvety/ui/use-html-dark-theme` -> `useHtmlDarkTheme`, `readHtmlDarkTheme`: Subscribes to `html.dark` (`next-themes` class strategy). Used on the gateway for Hyperspeed and hero marketing copy (headline, three shadcn `Badge` company-values pills with Lucide icons, including **`text-brand-swiss-red`** on the “Switzerland” span via `--brand-swiss-red` in `globals.css`); also works outside navbar-only `ThemeProvider` when client code must read the resolved theme from `html` (blocking init runs in `<head>` on all public shells).
