...
- Cross-app navigation: **`AppSwitcher`** reads product sections from `@helvety/shared/helvety-ecosystem-sections` via [`app-switcher-sections.tsx`](src/app-switcher-sections.tsx) and passes **absolute** `urls.*` hrefs into **`next/link`**, so each zone’s Next **`basePath`** does not rewrite links to other apps. Core Apps **Store** uses **`urls.storeProducts`**. Core Apps **Cloud** links to **`urls.cloud`** (`https://helvety.cloud`). The switcher sheet uses **`SHEET_SCROLLABLE_SHELL_CLASS`** and scrolls long link lists in **`ScrollArea`**.
