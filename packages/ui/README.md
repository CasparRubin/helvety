...
- `@helvety/ui/helvety-shell-route-loading` -> `HelvetyShellRouteLoading`: Full-viewport `bg-background` wrapper around `LoadingSpinner` for root `app/loading.tsx` on **`web`**, **`auth`**, and **`store`** (avoids white flash during client navigations; matches auth/store static fallback in light mode, below md, or reduced motion).
- `@helvety/ui/use-html-dark-theme` -> `useHtmlDarkTheme`, `readHtmlDarkTheme`: Subscribes to `html.dark` (`next-themes` class strategy). Used by shell backdrop and gateway hero gating; works outside navbar-only `ThemeProvider` (Store).
