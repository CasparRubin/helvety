...
- `@helvety/shared/entity-links-client` — client-safe Supabase helpers for `entity_links` (`createEntityLink`, `deleteEntityLink`, `getEntityLinksForEndpoint`, `toLinkedEntityReferences`). Intended for browser clients that talk to PostgREST directly (for example a future Chromium extension side panel); not wired in `packages/extension-chrome` yet. Zone server actions should still call `entity-link-action-primitives` for CSRF-protected mutations.
