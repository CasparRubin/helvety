...
- `@helvety/shared/entity-links-client` — client-safe Supabase helpers for `entity_links` (`createEntityLink`, `deleteEntityLink`, `getEntityLinksForEndpoint`, `toLinkedEntityReferences`). Used by the Chromium extension side panel and other browser clients that talk to PostgREST directly. Zone server actions should still call `entity-link-action-primitives` for CSRF-protected mutations.
