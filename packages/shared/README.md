...
- App `lib/env.ts` modules use tiered factories from `@helvety/shared/env-validation`: `createAppServerUpstashEnv` (admin tier), `createAppUserScopedEnv` (E2EE + docs), `createAppUpstashCookieEnv` (public tools), `getValidatedGatewayEnv` (web)
...
- Refresh auth session cookies early when `sb-*` cookies are present (`refreshSupabaseAuthSession` in `createSecurityProxy`, and on `createAppProxy` root → `basePath` redirects when session cookies are present). Edge verification prefers `auth.getClaims()` when the installed `supabase-js` exposes it, otherwise `getUser()`.
- After the proxy persists refreshed session cookies via `setAll`, it sets `x-helvety-auth-refreshed: 1` on the request and forwards it to Server Components. `createServerSupabaseClient` (alias: `createServerComponentClient`) then **no-ops** `setAll` so RSC does not attempt disallowed cookie writes; use `createServerMutatingClient` in route handlers and server actions that create or clear sessions (`exchangeCodeForSession`, `verifyOtp`, `signOut`, `updateUser`, etc.). Development still throws if the header is absent and an RSC write is blocked (guardrail for missing proxy refresh).
- CSRF cookie signing uses `HELVETY_COOKIE_SIGNING_SECRET` only (not `SUPABASE_SECRET_KEY`). Required on apps whose proxy profile sets `includeCsrf: true`: `e2ee-app`, `auth-gateway`, `store-gateway`, and `public-tool` (including `apps/docs`, which uses the user-scoped + Upstash env tier for vault APIs). The proxy re-issues the signed `csrf_token` cookie when it is missing or fails validation (for example after secret rotation); layouts read the current token via `getCachedCSRFToken` (cookie, then `x-csrf-bootstrap-token`). The gateway (`public-marketing`) does not bootstrap CSRF cookies. Auth also sets `helvety_device_trust` via `DEVICE_TRUST_COOKIE_SECRET` after email verification (passkey-first return sign-in on all `/auth/login` entry paths; documented in Privacy §9; see [`docs/cookies-telemetry-and-footer.md`](../../docs/cookies-telemetry-and-footer.md)). E2EE apps and Docs optional vault save cache derived master keys in IndexedDB (`helvety-crypto`) per `vault-session.ts` (12h sliding idle, 30d max).
