...
- After a successful proxy refresh, the proxy sets `x-helvety-auth-refreshed: 1` on the request and forwards it to Server Components. `createServerSupabaseClient` (alias: `createServerComponentClient`) then **no-ops** `setAll` in layouts so RSC does not attempt disallowed cookie writes; development still throws if the header is absent and a write is blocked (guardrail for missing proxy refresh).
