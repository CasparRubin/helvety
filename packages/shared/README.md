...
- Card-level Helvety Store catalog via `@helvety/shared/store-catalog` (`STORE_PRODUCT_CARDS`, `StoreProductId` literal-id union, plus typed sort/lookup helpers) for card fields (name, blurb, type, runs-on, free / open-source flags, release date). `category` is derived from the ecosystem registry by `storeProductSlug`; badge UI is in `apps/store/components/products/product-badge.tsx`; see `apps/store/README.md` › "Adding a New Product" for the end-to-end add-a-product flow
