...
- Next.js product metadata via `@helvety/shared/seo` (`createHelvetyProductMetadata`, `AI_DISCOVERY_USER_AGENTS`, `GATEWAY_DISALLOWED_PATHS`, plus sitemap/robots factories with explicit AI-crawler rules on public vs private zones) and shared SEO blurbs in `@helvety/shared/app-product-descriptions` (`WEB_SITE_DESCRIPTION`, per-app `*_DESCRIPTION`, PWA variants). Canonical crawl policy is gateway `/robots.txt` (RFC 9309); zone robots files are host-absolute mirrors. Public sitemaps list indexable page URLs only; private zones omit `app/sitemap.ts`. Vitest guardrails: `seo-zone-consistency.test.ts`, `seo.test.ts`, and per-app `seo-routes.test.ts` helpers in `@helvety/shared/test-utils/seo-route-test-helpers`.
- Company and licensing copy constants in `@helvety/shared/licensing` (`HELVETY_COMPANY_VALUES_TAGLINE`, `HELVETY_SWISS_ORIGIN_SEO`, `HELVETY_SWISS_ORIGIN_COUNTRY` for inline hero headlines, `HELVETY_WEB_DEFAULT_TITLE`, plus monorepo-specific AGPL helpers and repository-agnostic published-source helpers for Store/legal/`llms.txt` licensing sections)
...
- Helvety ecosystem product sections via `@helvety/shared/helvety-ecosystem-sections` (`HELVETY_ECOSYSTEM_PRODUCT_SECTIONS`, category titles, `ecosystemCategoryForStoreSlug`, `ecosystemItemHref`): single source of truth for app switcher grouping, store filter pills, and category badges. Register new products here first.
- Card-level Helvety Store catalog via `@helvety/shared/store-catalog` (`STORE_PRODUCT_CARDS`, `StoreProductId` literal-id union, `StoreProductType` delivery model, plus typed sort/lookup helpers) for card fields (name, blurb, type, runs-on, free / open-source flags, release date). `category` is derived from the ecosystem registry by `storeProductSlug`; `apps/store/lib/types/products.ts` imports `StoreProductType` for full `Product` rows; badge UI is in `apps/store/components/products/product-badge.tsx`; see `apps/store/README.md` › "Adding a New Product" for the end-to-end add-a-product flow
...
- `@helvety/shared/e2ee-entity-catalogs` — display catalogs (stage/label/category colors, priority metadata) for E2EE pickers and list grouping; `CONTACT_CATEGORIES` is a re-export alias of `NOTE_CATEGORIES`; consumed by zone `default-*.ts` and the Chromium extension (`consistency:e2ee-catalogs`).
...
- `@helvety/shared/test-utils/seo-route-test-helpers` — `expectPublicCrawlerRobots` for public-zone `robots.ts` tests (`*` plus `AI_DISCOVERY_USER_AGENTS`); `expectPrivateZoneRobots(robots(), "/zone")` for private zones
