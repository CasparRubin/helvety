import canvasSize from "./canvas-size-client";

/** Browser canvas backing-store limits used to clamp export dimensions. */
...
/** Normalized row returned by canvas-size probe helpers. */
...
/** Type guard for canvas-size probe results. */
...
/** Runs a canvas-size probe, returning a conservative fallback on failure. */
...
/** Rounds to the nearest even integer (minimum 1) for stable bitmap sizes. */
 ...
 * backing-store limits from {@link getCanvasExportLimitsCached}.
 */
...
/** Probes the current browser for usable canvas export limits. */
...
/** Cached browser canvas limits for export clamping (SSR-safe defaults). */
