...
export async function savePasskeyParams(params: {
  ...
  try {
    ...
    if (error) {
      console.error('Error saving passkey params:', error)
      return { success: false, error: "Failed to save passkey encryption settings" }
    }

    return { success: true }
  } catch (error) {
    console.error('Unexpected error in savePasskeyParams:', error)
 ...
 */
export async function getPasskeyParams(): Promise<EncryptionActionResponse<UserPasskeyParams | null>> {
  try {
    const supabase = await createClient()
    ...
    if (error) {
      ...
      console.error('Error getting passkey params:', error)
      return { success: false, error: "Failed to get passkey encryption settings" }
    }

    return { success: true, data: data as UserPasskeyParams }
  } catch (error) {
    console.error('Unexpected error in getPasskeyParams:', error)
    return { success: false, error: "An unexpected error occurred" }
  }
}
 ...
 */
export async function hasPasskeyEncryptionSetup(): Promise<EncryptionActionResponse<boolean>> {
 ...
 */
export async function getEncryptionParams(): Promise<EncryptionActionResponse<{
  type: 'passkey' | null
  ...
  try {
    ...
    console.error('Unexpected error in getEncryptionParams:', error)
    return { success: false, error: "An unexpected error occurred" }
  }
}
