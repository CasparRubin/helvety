...
| **[@helvety/shared](packages/shared/)** | Shared libraries: Supabase clients, auth, CSRF, proxy, rate limiting, crypto, cached server helpers, structured logging (`@helvety/shared/logger`, including `logUnexpectedError` for caught/API failures), ID helpers (`@helvety/shared/uuid-string`), **shared constants** (e.g. `ACTION_LIMITS` for reorder/export caps, toast durations), **`client-search`** (normalized in-browser list matching for E2EE apps), font definitions, types, utilities |
| **[@helvety/ui](packages/ui/)**         | Shared UI components: shadcn/ui, footer, theme provider, Tiptap editor, **ListSearchField** (main-list client search), CSRF provider, EncryptionGate, AppSwitcher, ThemeSwitcher, navbar-scoped animated Lucide icon aliases, AuthTokenHandler, SessionRecovery, SkipToContent (Vitest tests for selected primitives)                                                                                                                                     |
...
- Quality checks: `bun run ci:check` runs `format:check`, `lint`, `type-check`, and `test`. For a full pre-deploy pass including production build, run `bun run ci:release` (same as `ci:check` plus `build`). Root `lint`, `type-check`, `test`, and `build` scripts delegate to Turbo across workspaces. **`ci:release` needs real env** (see each app’s `env.template`): gateway rewrite URLs for Web, Supabase and Upstash for server apps, etc. If you have no local `.env`, run **`bun run ci:release:stub`** instead: it runs the same checks as `ci:release`, then runs **`build` with `SKIP_ENV_VALIDATION=1`** so Next.js can compile using schema-valid placeholders (ignored when `VERCEL=1`). Earlier steps do not set the flag so Vitest and env-validation tests behave normally. Vercel deployments must use real project env vars, not `SKIP_ENV_VALIDATION`. On a machine with full `.env` files, `ci:release` is the closest match to what Vercel runs before promoting a deployment.
