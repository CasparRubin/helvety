...
| **[@helvety/shared](packages/shared/)** | Shared libraries: Supabase clients, auth, CSRF, proxy, rate limiting (`authenticateAndRateLimit`, `RATE_LIMITS` including export presets), crypto, cached server helpers, structured logging (`@helvety/shared/logger`, including `logUnexpectedError` for caught/API failures), ID helpers (`@helvety/shared/uuid-string`), **shared constants** (e.g. `ACTION_LIMITS` for reorder/export row caps, toast durations), **E2EE page guard** (`e2ee-page-auth`), **`client-search`** (normalized in-browser list matching for E2EE apps), font definitions, types, utilities |
| **[@helvety/ui](packages/ui/)**         | Shared UI: shadcn/ui, footer, theme provider, Tiptap, **ListSearchField** (E2EE list search), CSRF provider, EncryptionGate / **EncryptionGateApp**, **E2eeAppRootLayout** and **E2eeAppNavbar** (shared Contacts/Notes/Tasks shell), **RootGlobalError**, **useE2eeEntityListDndSensors**, AppSwitcher, ThemeSwitcher, Lucide icons (`lucide-react`), AuthTokenHandler, SessionRecovery, SkipToContent, Vercel analytics helpers (Vitest on selected primitives)                                                                                                          |
...
- Quality checks: `bun run ci:check` runs `format:check`, `lint`, `type-check`, and `test`. For a full pre-deploy pass including production build, run **`bun run ci:release`** (`ci:check` plus `build`). The **`build`** step sets **`SKIP_ENV_VALIDATION=1`** so Next.js can compile without a local `.env`: `@helvety/shared/env-validation` uses schema-valid placeholders for any **missing** `NEXT_PUBLIC_*` / server / Upstash values, and still **validates real values when they are present** (so a machine with full `.env` files exercises the same keys as production). **`VERCEL=1` disables placeholder mode entirely**—Vercel builds must use real project env vars. Earlier `ci:release` steps do not set `SKIP_ENV_VALIDATION` so Vitest and env-validation tests behave normally.
...
- Workspace dependency drift check: run `bun run deps:drift` to enforce lockstep versions for critical stack dependencies across `apps/*` and `packages/*`.
- Dependency policy and override hygiene rules are documented in [`DEPENDENCY_GOVERNANCE.md`](DEPENDENCY_GOVERNANCE.md).
...
- If you deploy behind custom reverse proxies, set `HELVETY_SERVER_ACTION_ALLOWED_ORIGINS` (comma-separated origins) so Next.js Server Actions only trust explicitly allowed origins.
...
Architecture entry points are also documented in [`ARCHITECTURE.md`](ARCHITECTURE.md).
