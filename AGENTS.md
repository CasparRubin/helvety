...
- Session-aware login on `/auth/login` (`resolveAuthenticatedEmailBootstrap` in `apps/auth/lib/login-email-bootstrap.ts`) may skip the passkey step for **non-E2EE** destinations only; **`requiresE2eeBrowserUnlock`** in `@helvety/shared/e2ee-app-paths` keeps notes/tasks/contacts on passkey sign-in until local crypto unlock. **`EncryptionGate`** redirects to auth with `force_login=1` via `redirectToLoginOnce` when unlock is needed.
